...
const ChatWithAi = () => {
  ...
  const chatWithAiAnalysisContinue = localStorage.getItem(
    "ChatWithAiVideoAnalysisContinue"
  );

  const videoAnalysisYesNoMaybeHandler = async (ans: number) => {
    localStorage.setItem("ChatWithAiVideoAnalysisDone", "true");

    const yseText = `The video analysis generated by ChatGPT for the 2024 US presidential debate was useful.`;
    const noText = `The video analysis generated by ChatGPT for the 2024 US presidential debate was not useful.`;
    const maybeText = `I am not sure if the video analysis generated by ChatGPT for the 2024 US presidential debate is useful or not.`;
    ...
    if (ans == 1) {
      ...
      await axios
        .post(
          baseUrl + `/chat-with-ai/get-response`,
          { prompt: yseText },
          {
        ...
        .then((res) => {
          setMessages([
            ...messages,
            { message: res.data.data.message, prompt: yseText },
          ]);
        });
    } else if (ans == 2) {
      setMessages([
      ...
      await axios
        ...
        .then((res) => {
          setMessages([
            ...messages,
            { message: res.data.data.message, prompt: noText },
          ]);
        });
    } else {
      setMessages([
      ...
      await axios
        ...
        .then((res) => {
          setMessages([
            ...messages,
            { message: res.data.data.message, prompt: maybeText },
          ]);
        });
    }
  };

  const chatWithAiAnalysisDone = localStorage.getItem(
    "ChatWithAiVideoAnalysisDone"
  );
  ...
  const introVideoFirstRequire = () => {
    toast.error("Please see the video first then press Analyze the video.");
  };
